...
outDir <- "/home/RNAseq/pancan/webdata/ClinAS/ssplots/"
clin <- readRDS(file=paste(fileDir,"clinical_variates_with_sample_more_than20_spliceseq.rds",sep=""))
ls <- readRDS(paste(fileDir,"spliceseq_clinical_variates_as_PSIdata.rds",sep=""))
infoAS <- read.csv((paste(fileDir,"spliceseq_clinical_variates_as_info_10.csv",sep="")))
...
CIplot=function(infos){
  ...
  psi <- ls[[CancerType]]
  psi$sampleID <- row.names(psi)
  
  psi <- psi[,c("sampleID",SpliceEvent)]
  names(psi)[2] <- "PSI"
  df <- merge(ci,psi,by="sampleID")
  ...
  title <- paste("SpliceSeq - ",CancerType,"\n",SpliceEvent,"\nP-value = ",p,sep="")
