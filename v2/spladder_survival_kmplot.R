...
KMplot=function(infos){
  ...
  idType <- infoX[infoX$SpliceEvent==SpliceEvent,"idType"]
  ...
  if (!is.na(med)){
    if(!is.na(fit)){
      # osPrev <- c("spladder_med_data_os_","spladder_fit_data_os_")
      data[[1]] <- meddata[,c("timeY","event",SpliceEvent)]
      data[[2]] <- fitdata[,c("timeY","event",SpliceEvent)]
      ncol=2
      width=8
    }else if(is.na(fit)){
      # osPrev <- c("spladder_med_data_os_")
      data[[1]] <- meddata[,c("timeY","event",SpliceEvent)]
      ncol=1
      width=4
    }
    ...
    suppressWarnings(ggsave(p, file = outFilePath,width=width,height=3))
...
for (tt in types){
  ...
  load(paste(fileDir,"spladder_med_data_",survType,"_",cancerType,".Rdata",sep = ""))
  meddata <- mydata
  load(paste(fileDir,"spladder_fit_data_",survType,"_",cancerType,".Rdata",sep = ""))
  fitdata <- mydata
